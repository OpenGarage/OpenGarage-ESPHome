...
class Secplus1Cover : public PulseCover {
 public:
...
// One bounded light intent, not an action/retry queue. Toggle wire commands need
// a fresh known baseline, are idempotent against that baseline, and are never retried.
class Secplus1LightIntent {
 public:
  explicit Secplus1LightIntent(Secplus1Transport &port) : port_(port) {}
