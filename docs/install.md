...
1. Download the ESPHome firmware file ending in `-esp8266-ota.bin` from [GitHub Releases](https://github.com/OpenGarage/OpenGarage-ESPHome/releases), under **Assets**. Check the downloaded file against its published SHA-256 checksum. Follow the stock OG [firmware update instructions](https://opengarage.github.io/OpenGarage-Firmware/#firmware-update-instructions) to upload it, but **skip their stock firmware download step and use the ESPHome `.bin` downloaded above**. Installation replaces the stock firmware, UI and cloud features.
